...
import { BASE_SLOT_COLS, BASE_SLOT_ROWS, BLOCKADE_COOLDOWN, BLOCKADE_DURATION, COLLECT_MIN_AMOUNT, } from './config.js';
...
export class HomeBase {
    ...
    constructor(zone, color, scene) {
        ...
        const floor = new THREE.Mesh(new THREE.PlaneGeometry(w, d), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.14, depthWrite: false }));
        ...
        this.blockadeButtonPos = new THREE.Vector3(zone.entrance.x + zone.side * 0.8, zone.floorY, zone.rect.minZ + 1.6);
        this.blockadeButton = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 0.1, 20), new THREE.MeshStandardMaterial({
            color: 0xb03030,
            emissive: 0x801818,
        ...
        for (let i = 0; i < 5; i++) {
            const beam = new THREE.Mesh(beamGeo, new THREE.MeshBasicMaterial({ color: 0xff2222, transparent: true, opacity: 0.85 }));
        ...
        this.sign = new TextSprite('Basis', { worldHeight: 1.0, fontSize: 52, background: 'rgba(0,0,0,0.4)' });
    ...
    incomePerSec() {
        ...
        for (const s of this.slots)
            if (s.item)
                total += s.item.rarity.incomePerSec;
        return total;
    }
    storedCount() {
    ...
    bestStoredItem() {
        let best = null;
        for (const s of this.slots) {
            if (s.item && (!best || s.item.rarity.incomePerSec > best.rarity.incomePerSec))
                best = s.item;
        }
        return best;
    }
    /** Place an item into a specific slot index (or the first free one). */
    deposit(item, slotIndex = -1) {
        let idx = slotIndex;
        if (idx < 0)
            idx = this.slots.findIndex((s) => !s.item && !s.reservedFor);
        if (idx < 0)
            return false;
    ...
    stealFrom(item) {
        ...
        if (idx < 0 || this.slots[idx]?.item !== item)
            return;
        const slot = this.slots[idx];
        slot.item = null;
    ...
    activateBlockade() {
        if (this.blockadeActive || this.blockadeCooldown > 0)
            return false;
    ...
    update(dt, time) {
        for (const s of this.slots) {
            if (s.item)
                s.accrued += s.item.rarity.incomePerSec * dt;
        }
        ...
        if (this.labelTimer <= 0) {
            ...
            for (const s of this.slots) {
                ...
                if (show)
                    s.accruedLabel.setText(formatMoney(Math.floor(s.accrued)));
                const mat = s.button.material;
                mat.emissiveIntensity = show ? 0.7 + Math.sin(time * 5) * 0.3 : 0.15;
            }
            const bMat = this.blockadeButton.material;
