...
import { BRAINROTS } from './catalog.js';
import { PLAYER_HEIGHT } from './config.js';
import { baueFigur } from './figur.js';
function setShadows(obj, cast, receive) {
    obj.traverse((c) => {
        const mesh = c;
        if (mesh.isMesh) {
 ...
 */
function bakeToStatic(template) {
 ...
 */
function normalizeIntoPivot(obj, targetHeight) {
...
function findBone(root, ...needles) {
    ...
    root.traverse((c) => {
        ...
        const ok = needles.every((n) => n.startsWith('!') ? !name.includes(n.slice(1).toLowerCase()) : name.includes(n.toLowerCase()));
        if (ok)
            found = b;
    });
    return found;
...
export class AssetManager {
    ...
    async loadAll(onProgress) {
        ...
        const mapP = gltf.loadAsync(assetBase + 'brainrotmap.glb').then((g) => {
            this.mapScene = g.scene;
            tick('Map');
        });
        const baseP = gltf.loadAsync(assetBase + 'Base1floor.glb').then((g) => {
            this.baseTemplate = g.scene;
            tick('Base building');
        });
        /* Die Figur wird gebaut, nicht geladen — siehe figur.js. Ohne
           Animationsspuren schaltet character.js auf die gerechnete
           Bewegung um, und das spart nebenbei eine Viertelmegabyte. */
        const playerP = Promise.resolve().then(() => {
            const f = baueFigur();
            this.playerTemplate = f.scene;
            this.playerClips = f.animations;
            tick('Figur');
        });
        const batP = gltf.loadAsync(assetBase + 'simple_bat.glb').then((g) => {
            ...
            this.batTemplate = bakeToStatic(g.scene);
            tick('Bat');
        });
        const brainrotPs = BRAINROTS.map((def) => fbx.loadAsync('./' + def.file).then((obj) => {
    ...
    applyCalibration(pivot, key, target) {
    ...
    makeCharacter(tint) {
        const cloned = skeletonClone(this.playerTemplate);
        ...
        if (tint !== undefined) {
            ...
            cloned.traverse((c) => {
                ...
                const recolor = (m) => {
                    const copy = m.clone();
                    if (copy.color)
                        copy.color.offsetHSL(shift, 0, 0);
                    return copy;
                };
    ...
    makeBrainrot(defId, targetHeight) {
        const tpl = this.brainrotTemplates.get(defId);
        if (!tpl)
            throw new Error('Unknown brainrot id: ' + defId);
        const cloned = skeletonClone(tpl.obj);
        setShadows(cloned, true, false);
