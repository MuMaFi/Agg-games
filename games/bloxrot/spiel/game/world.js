...
import { BASE_FOOTPRINT, BASE_GAP_FROM_CARPET, MAP_SCALE } from './config.js';
...
export function pushCircleOutOfRect(pos, radius, r) {
...
export class World {
    ...
    build(assets, basesNeeded) {
        ...
        const findMesh = (needle) => {
            ...
            map.traverse((c) => {
                if (found)
                    return;
                const mesh = c;
                if (mesh.isMesh && squash(mesh.name).includes(n))
                    found = mesh;
            });
            return found;
        };
        ...
        const carpetBox = carpetMesh ? boxOf(carpetMesh) : new THREE.Box3(new THREE.Vector3(-2.4, 0, -30), new THREE.Vector3(2.4, 0.8, 30));
        const carpetRect = {
        ...
        for (const side of [1, -1]) {
            for (let i = 0; i < perSide; i++) {
                ...
                const fb = boxOf(inst);
                const footprint = { minX: fb.min.x, maxX: fb.max.x, minZ: fb.min.z, maxZ: fb.max.z };
                const rect = {
    ...
    groundHeightAt(x, z) {
        for (const b of this.layout.bases) {
            if (rectContains(b.footprint, x, z, 0.2))
                return b.floorY;
        }
        if (rectContains(this.layout.carpetRect, x, z))
            return this.layout.carpetY;
    ...
    baseIndexAt(x, z, pad = 0) {
        for (let i = 0; i < this.layout.bases.length; i++) {
            if (rectContains(this.layout.bases[i].rect, x, z, pad))
                return i;
        }
        return -1;
    }
