...
import { rectContains } from './world.js';
import { BOT_ATTACK_RANGE, BOT_CHASE_LEASH, BOT_COLLECT_THRESHOLD, BOT_DEFEND_RADIUS, BOT_THINK_INTERVAL, PICKUP_RADIUS, } from './config.js';
/**
...
export class BotController {
    ...
    api;
    state = 'decide';
    thinkTimer = Math.random() * BOT_THINK_INTERVAL;
    waypoint = null;
    targetItem = null;
    targetEntity = null;
    wanderTimer = 0;
    ...
    think() {
        ...
        if (this.state === 'harvest' && this.targetItem && this.targetItem.state === 'conveyor'
            && me.money >= this.targetItem.rarity.price)
            return;
        if (this.state === 'raid' && this.targetItem && this.targetItem.state === 'stored'
            && this.targetItem.storedIn && this.targetItem.storedIn !== this.home)
            return;
    ...
    bestRaidTarget() {
        ...
        for (const other of this.api.entities) {
            ...
            if (!base)
                continue;
            if (base.blockadeActive)
                continue; // lasers up: not worth the trip
            ...
            if (other.isStunned)
                score += 25; // owner is on the floor: prime time
            if (score > bestScore) {
                bestScore = score;
                best = item;
            }
    ...
    act(dt) {
        ...
        switch (this.state) {
            ...
            case 'defend': {
                ...
                const targetGone = target.position.distanceTo(this.home.zone.center) > BOT_CHASE_LEASH &&
                    target.carried?.stolenFromBase !== this.home;
                const stillThreat = target.carried?.stolenFromBase === this.home ||
            ...
            case 'wander': {
                this.wanderTimer -= dt;
                if (!this.waypoint || this.wanderTimer <= 0)
                    this.pickWanderPoint();
                this.steerTo(this.waypoint);
    ...
    opportunisticSwing() {
        ...
        for (const other of this.api.entities) {
            ...
            if (d > BOT_ATTACK_RANGE)
                continue;
            const isThreat = other.carried?.stolenFromBase === this.home ||
                other.isSwinging ||
