...
import { HomeBase } from './base.js';
import { Conveyor } from './conveyor.js';
import { Entity } from './character.js';
import { PlayerController } from './player.js';
import { BotController } from './bot.js';
import { Hud } from './hud.js';
import { FloatingText, MoneyBurst } from './textSprite.js';
import { formatMoney } from './catalog.js';
import { SoundFx } from './audio.js';
import { pushCircleOutOfRect } from './world.js';
import { BAT_ARC_DEG, BAT_RANGE, BASE_SLOTS, BLOCKADE_BUTTON_RADIUS, BOT_SPEED, BOT_STARTING_MONEY, COLLECT_BUTTON_RADIUS, COLLECT_MIN_AMOUNT, ENTITY_RADIUS, PICKUP_RADIUS, PLAYER_SPEED, STARTING_MONEY, } from './config.js';
const BOT_ROSTER = [
...
export class Game {
    ...
    async start() {
        ...
        this.conveyor.onSpawn = (item) => {
            ...
            if (item.rarity.incomePerSec >= 35) {
                this.hud.notify(`✨ ${item.rarity.label} ${item.def.name} liegt auf dem Band!`, item.rarity.color);
            }
        };
        ...
        this.hud.notify('Kauf vom roten Teppich (E), stell sie in deine Basis — und klau bei allen anderen!', '#ffd24d');
        this.clock.start();
    ...
    tryBuy(buyer, item) {
        if (item.state !== 'conveyor' || buyer.carried)
            return false;
        if (buyer.money < item.rarity.price)
            return false;
        ...
        buyer.pickUp(item);
        if (buyer.isPlayer) {
            this.hud.notify(`${item.def.name} für ${formatMoney(item.rarity.price)} gekauft — ab nach Hause!`, item.rarity.color);
        }
        else if (item.rarity.incomePerSec >= 35) {
            this.hud.notify(`${buyer.name} hat ${item.def.name} gekauft!`, item.rarity.color);
        }
        return true;
    }
    /** Steal a stored item out of someone else's base (player & bots). */
    trySteal(thief, item) {
        if (item.state !== 'stored' || thief.carried)
            return false;
        const base = item.storedIn;
        if (!base || base.owner === thief)
            return false;
        base.stealFrom(item);
        this.world.scene.remove(item.root);
        thief.pickUp(item);
        ...
        if (victim.isPlayer)
            this.audio.alarm();
        this.hud.notify(thief.isPlayer
            ? `🦹 You grabbed ${victim.name}'s ${item.def.name}! RUN!`
            : `🚨 ${thief.name} is stealing ${victim.isPlayer ? 'YOUR' : victim.name + "'s"} ${item.def.name}!`, item.rarity.color);
        return true;
    }
    /** Free pickup of an item lying on the ground (dropped, never stolen). */
    tryPickupGround(who, item) {
        if (item.state !== 'ground' || who.carried)
            return false;
    ...
    dropCarried(victim, attacker) {
        ...
        if (item.stolenFromBase) {
            ...
            item.startReturnFlight(target, () => {
                ...
                this.hud.notify(`🏠 ${item.def.name} ist zu ${home.owner.name} zurückgeflogen!`, item.rarity.color);
            });
            this.hud.notify(attacker.isPlayer
                ? `💥 You batted ${victim.name} — the ${item.def.name} is flying home!`
                : `💥 ${attacker.name} batted ${victim.name} and saved the ${item.def.name}!`, '#ff8c4d');
        }
        else {
            ...
            this.hud.notify(`💥 ${victim.name} hat ${item.def.name} fallen lassen!`, '#ff8c4d');
        }
    }
    ...
    handleDeposits() {
        for (const e of this.entities) {
            ...
            if (e.position.x < zone.rect.minX - 0.5 || e.position.x > zone.rect.maxX + 0.5)
                continue;
            if (e.position.z < zone.rect.minZ - 0.5 || e.position.z > zone.rect.maxZ + 0.5)
                continue;
            if (!e.home.hasFreeSlot())
                continue;
            const stolenFrom = item.stolenFromBase;
            if (stolenFrom)
                stolenFrom.releaseReservation(item);
            e.releaseCarried();
            this.world.scene.add(item.root);
            e.home.deposit(item);
            this.audio.deposit(e.position.distanceTo(this.player.position));
            if (e.isPlayer) {
                this.hud.notify(`🏦 ${item.def.name} eingelagert! +${formatMoney(item.rarity.incomePerSec)}/s — step its button to collect`, item.rarity.color);
            }
            else if (stolenFrom?.owner.isPlayer) {
                this.hud.notify(`😡 ${e.name} ist mit DEINEM ${item.def.name} abgehauen!`, '#ff5a5a');
            }
        }
    }
    ...
    handleButtons() {
        for (const base of this.bases) {
            ...
            for (const slot of base.slots) {
                if (slot.accrued < COLLECT_MIN_AMOUNT)
                    continue;
                const dx = owner.position.x - slot.buttonPos.x;
                const dz = owner.position.z - slot.buttonPos.z;
                if (dx * dx + dz * dz > COLLECT_BUTTON_RADIUS * COLLECT_BUTTON_RADIUS)
                    continue;
            ...
            if (base.blockadeReady) {
                ...
                if (dx * dx + dz * dz <= BLOCKADE_BUTTON_RADIUS * BLOCKADE_BUTTON_RADIUS) {
                    if (base.activateBlockade()) {
                        ...
                        if (owner.isPlayer) {
                            this.hud.notify('🔒 Sperre steht! Der Eingang ist dicht.', '#ff6666');
                        }
                        else if (base.zone.center.distanceTo(this.player.position) < 25) {
                            this.hud.notify(`🔒 ${owner.name} hat die Basis dichtgemacht!`, '#ff8a8a');
                        }
                    }
                }
            }
    ...
    handleCombat() {
        ...
        for (const attacker of this.entities) {
            if (!attacker.consumeSwingImpact())
                continue;
    ...
    tick() {
        ...
        if (this.playerCtl.consumeSwing())
        ...
        if (wantsInteract && target) {
            if (target.kind === 'buy')
                this.tryBuy(this.player, target.item);
            else if (target.kind === 'steal')
                this.trySteal(this.player, target.item);
            else
                this.tryPickupGround(this.player, target.item);
        }
        // effects
        ...
        if (this.hudTimer <= 0) {
            ...
            this.hud.setBlockade(home.blockadeActive ? 'active' : (home.blockadeReady ? 'ready' : 'cooldown'), home.blockadeTimeLeft);
            this.hud.updateLeaderboard(this.entities);
            this.hud.setPointerLocked(this.playerCtl.pointerLocked || !!this.playerCtl.daumen?.aktiv);
            this.hud.setPrompt(this.player.carried
                ? null
                : (target ? target.label : null));
            const carried = this.player.carried;
            this.hud.setCarry(carried
